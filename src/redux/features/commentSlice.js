...
export const actFetchAllComments = createAsyncThunk(
  ...
  async (params = {}) => {
    ...
    return {
      data: response.data, // Dữ liệu nhận xét
...
export const actFetchCommentById = createAsyncThunk(
  ...
  async (id) => {
    const response = await commentApis.getCommentById(id); // Gọi API để lấy nhận xét theo ID
...
const commentSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(actFetchAllComments.fulfilled, (state, action) => {
      state.isLoading = false; // Tắt trạng thái tải dữ liệu
      state.comments = action.payload.data; // Cập nhật danh sách nhận xét
    ...
    builder.addCase(
      ...
      (state, action) => {
        state.isLoading = false; // Tắt trạng thái tải dữ liệu
        state.commentsCalcuStarAverage = action.payload; // Cập nhật danh sách nhận xét để tính trung bình sao
      }
    );
